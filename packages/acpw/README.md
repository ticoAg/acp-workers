...
默认打印 markdown。脚本要 JSON 时加 `--json`，或 `acpw output set json`。`ok` 表示 ACP 回合结束，不表示任务做对。
...
原生模式是 `48190` 上的一条 WebSocket，底下挂多个 agent 进程。`acpw up` 起它；`acpw run NAME` 开或续一段会话；`acpw down NAME` 停一个 child；`acpw down` 停这条 socket。Zed / acp-devtools 走 `acpw stdio NAME`（标准 ACP stdio，见仓库根 [README](../../README.md#和-acp-devtools-合用)）。Pool 里的 grok 是 `grok agent --always-approve --no-leader stdio`。`--no-pool` 是独立 gateway / `grok agent serve` 逃生口。线路契约见 [`docs/pool-protocol.md`](../../docs/pool-protocol.md)。
