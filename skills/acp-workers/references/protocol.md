...
Claude / Codex / Cursor have no Grok-style `serve`. Grok has both: `grok agent serve` (`acpw up grok --no-pool`) and `grok agent --always-approve --no-leader stdio` (the native pool child, same argv shape as acpx's `grok-build`). `acpw gateway` keeps one stdio ACP child and one in-flight WebSocket client. `initialize`/`authenticate` results are cached so the next `ping`/`run` does not re-handshake the child.

One client at a time is the limit that the shared WebSocket lifts: the same children sit under one daemon that serves many connections and many concurrent requests. A worker is poolable when it has a `stdio_argv`, regardless of its standalone transport. See [pool.md](pool.md).
