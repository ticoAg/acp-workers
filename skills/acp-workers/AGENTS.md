...
用户要把编码任务交给本机允许的 worker（`acpw allow` / `ls` 的 `allowed`）、起一条底下挂多个 agent 的 WebSocket（`acpw up`）、或用 `session_id` 续对话时，使用本 skill。
...
2. `bash scripts/ensure-acpw.sh`，然后 `acpw doctor && acpw ls`。原生路径是 `pool.live` / `via: pool`。只派 `enabled` 且 `allowed` 的 worker。
...
4. 写一件可验收的任务，用 `acpw run` 派给允许的 worker。
